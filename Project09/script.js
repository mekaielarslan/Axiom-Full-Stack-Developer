...
// Temorary array of transactions - to be replaced with local storage
const Transactions = [
...
function displayTransaction(transaction) {
    // Calculate if transaction is Cradit or Debit
    ...
    // Assign the inner HTML foe the transaction li
    transactionLI.innerHTML = `
    ${transaction.reason}<span>${transaction.amount}</span>
    <button class="delete-btn" onclick="deleteTransaction(${transaction.id}) ">X</button>
    `;
    // Add the li in DOM under the transaction history list
    list.appendChild(transactionLI);
};

// Function to update all balances
function updateBalance() {
    // Create  new  array with just the amounts
    ...
    const creditBalance = transactionAmounts
     ...
     const debitBalance = transactionAmounts
                            .filter(amount => amount < 0)
                            .reduce((acc, amount) => (acc += amount), 0);
    // Update values in the DOM for overall balance, creditBalance and debitBalance
...
// Functionto add a transaction from the form
function addTransaction(e) {
    ...
    if ( reason.value.trim() === '' || amount.value.trim() === '') {
        ...
        // Push the new transaction i the DOM
